...
public class Blog {
    ...
    public String handleRequest(@RequestParam(value="p")int p, Map<String, Object> map) {
        // 读取对应 blog(s)
        if (p == 0) {
            List<com.feilong.blog.dao.Blog> blogs = blogDao.getAll();
            map.put("blogs", blogs);
            return "blogs";
        } else {
            com.feilong.blog.dao.Blog blog = blogDao.getById(p);
            map.put("blog", blog);
            return "blog";
        }
    }
    ...
    public String handleCategoryRequest(@RequestParam(value="category")String category, Map<String, Object> map) {
        // 根据种类查找数据库
        List<com.feilong.blog.dao.Blog> blogs = blogDao.getByCategory(category);
        map.put("blogs", blogs);
        return "blogs";
    }
}
